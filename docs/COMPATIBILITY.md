...
BATFlow 0.5.0 models the Win98 MS-DOS 7.1 `COMMAND.COM` profile. Simulation is
an explanatory control-flow model; it does not execute imported commands and
is not a byte-for-byte interpreter.

The 0.5.0 trace is intentionally single-file. `CALL` is recorded and returns to
